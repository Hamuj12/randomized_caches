#!/usr/bin/env bash
# No -u here: conda deactivate hooks don't play nicely with nounset.
set -eo pipefail
...
# Use the no-libc binary to avoid SE-mode syscalls from glibc/ld.so
...
echo "[*] Running MIRAGE with no-libc test binary..."
"$GEM5" --outdir "$OUTDIR" \
  "$CFG" \
  --num-cpus=1 \
  --mem-size=8GB \
